...
import { PdfPreviewPopup } from "./pdfPreviewPopup"; // ajuste o caminho conforme a estrutura
...
export const PdfViewerModal = ({ norma, open, onOpenChange }: PdfViewerModalProps) => {
  ...
  // Gerenciamento do ESC e overflow
  useEffect(() => {
    if (!open) return;
      ...
      >
        ...
        <div className="flex items-center justify-between px-8 py-6 bg-primary border-b border-border">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">{norma.titulo}</h2>
              {norma.descricao && (
          ...
          <div className="w-full max-w-4xl h-full bg-background border-2 border-dashed border-border rounded-lg flex flex-col items-center justify-center gap-6 p-12 text-center">
            ...
            <div className="space-y-2">
              ...
              <p className="text-muted-foreground max-w-md">
                Para melhor compatibilidade e segurança, clique no botão abaixo
                para acessar o conteúdo completo em uma nova aba do navegador.
              </p>
            </div>
        ...
        <div className="px-8 py-4 bg-card border-t border-border">
          <p className="text-xs text-muted-foreground text-center">
            Pressione{" "}
            <kbd className="px-2 py-1 bg-muted rounded text-xs">ESC</kbd> para
            fechar • Última atualização:{" "}
            {new Date(norma.ultimaAtualizacao).toLocaleDateString("pt-BR")}
          </p>
        </div>
      </div>
    </div>
